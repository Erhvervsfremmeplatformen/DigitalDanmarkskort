...
const actions: ActionTree<FilterState, RootState> = {
  async getFacilities({ commit, dispatch }) {
    ...
    await axios
      .get(`https://api-ramboll.onltd.dk/v1/facilities`)
  ...
  filter({ state, commit }) {
    ...
    if (state.providerTypes.length > 0)
      facilities = facilities.filter(x =>
        checker(
          state.providerTypes.map(y => y.value),
          x.providerTypes.map(y => y.value)
        )
      );
    if (state.categories.length > 0)
      facilities = facilities.filter(x =>
        checker(
          state.categories.map(y => y.value),
          x.categories.map(y => y.value)
        )
      );
    if (state.areaTypes.length > 0)
      facilities = facilities.filter(x =>
        checker(
          state.areaTypes.map(y => y.value),
          x.areaTypes.map(y => y.value)
        )
      );
    if (state.serviceTypes.length > 0)
      facilities = facilities.filter(x =>
        checker(
          state.serviceTypes.map(y => y.value),
          x.serviceTypes.map(y => y.value)
        )
      );
